...
std::string SystemTrayIcon::Impl::make_tooltip_text() const
{
    ...
    return fmt::format(
        ...
        fmt::arg("upload_speed", tr_formatter_speed_Bps(tr_sessionGetSpeedBps(session, TR_UP))),
        fmt::arg("download_speed", tr_formatter_speed_Bps(tr_sessionGetSpeedBps(session, TR_DOWN))));
}
